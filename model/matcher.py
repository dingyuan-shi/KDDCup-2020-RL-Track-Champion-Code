...
from global_var import lng_step, lat_step, DIRS, DIR_NUM, INIT_VALUE, INIT_GRID, NUM_TOPK, TIME_INTERVAL, IS_CLEAR
...
class Matcher:
    def __init__(self, alpha, gamma):
        ...
        if INIT_GRID:
            self.grid_values = pickle.load(open(get_path(__file__, "values_grid.pkl"), "rb"))
            # print(f'grid_values: {self.grid_values}')
    ...
    def dispatch(self, dispatch_observ: List[Dict[str, Any]]) -> List[Dict[str, str]]:
        ...
        for key in pairs:  # type: str
            ...
            if done_prob > 0:
                ...
                # expected_update = done_prob * complete_update + (1 - done_prob) * cancel_update
                # expected_update = done_prob * order.reward + self.gamma * v1 - v0
    ...
    def update_value(self, loc: Tuple[float, float], grid_id: str, delta: float) -> None:
        ...
        for i, one_dir in enumerate(DIRS):
            layer_id = get_layer_id(loc[0] + one_dir[0] * lng_step, loc[1] + one_dir[1] * lat_step, direction = 0)
            # print(f'update:layer_id:{layer_id}')
